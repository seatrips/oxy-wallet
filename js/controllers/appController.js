...
angular.module('liskApp').controller('appController', ['oxyAPI', 'dappsService', '$scope', '$rootScope', '$http', "userService", "$interval", "$timeout", 'viewFactory', '$state', 'blockService', 'sendTransactionModal', 'registrationDelegateModal', 'serverSocket', 'delegateService', '$window', 'forgingModal', 'errorModal', 'userInfo', 'transactionsService', 'secondPassphraseModal', 'focusFactory', 'gettextCatalog',
	function (oxyAPI, dappsService, $rootScope, $scope, $http, userService, $interval, $timeout, viewFactory, $state, blockService, sendTransactionModal, registrationDelegateModal, serverSocket, delegateService, $window, forgingModal, errorModal, userInfo, transactionsService, secondPassphraseModal, focusFactory, gettextCatalog) {
		...
		$scope.getPriceTicker = function () {
			$http.get("https://explorer.oxycoin.io/api/getPriceTicker")
				.then(function (response) {
					$scope.btc_usd = response.data.tickers.BTC.USD;
					$scope.oxy_btc = response.data.tickers.OXY.BTC;
					$scope.oxy_usd = response.data.tickers.OXY.USD;
					$scope.oxy_eur = response.data.tickers.OXY.EUR;
				});
		};

		$scope.getVersion = function () {
			oxyAPI.peers.version()
				.then(function (peerVersionResponse) {
					return oxyAPI.blocks.getStatus()
				...
				.then(function (allResp) {
					...
					$scope.testnet = $scope.nethash === '0daee950841005a3f56f6588b4b084695f0d74aaa38b21edab73446064638552';
		...
		$scope.convertToCUR = function (shift, cur) {
			return (shift / 100000000) * $scope['oxy_' + (cur || usd)];
		};
		...
		$scope.getAppData = function () {
			if ($scope.fees === null) {
				$http.get('/api/blocks/getFees')
					.then(function(resp) {
						if (resp.data.success) {
							$scope.fees = resp.data.fees;
						}
					});
			}
