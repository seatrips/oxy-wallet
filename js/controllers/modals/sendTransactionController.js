...
angular.module('liskApp').controller('sendTransactionController', ['oxyAPI', '$scope', 'sendTransactionModal', '$http', 'userService', 'feeService', '$timeout', '$filter', function (oxyAPI, $scope, sendTransactionModal, $http, userService, feeService, $timeout, $filter) {
    ...
    $scope.isCorrectValue = function (currency, throwError) {
        ...
        if (amount == '') {
            return error('OXY amount can not be blank');
        }
        ...
        if (parts.length == 1) {
            ...
            if (parts[1].length > 8) {
                return error('OXY amount must not have more than 8 decimal places');
            ...
            return error('OXY amount must have only one decimal point');
        }
        ...
        if (amount == '0' && fraction == '00000000') {
            return error('OXY amount can not be zero');
        }
        ...
        if (!/^\d+$/.test(result)) {
            return error('OXY amount contains non-numeric characters');
        }
    ...
    $scope.convertOXY = function (currency) {
    ...
    $scope.sendTransaction = function (secretPhrase, withSecond) {
        ...
        var data = {
            secret: secretPhrase,
            amount: $scope.convertOXY($scope.amount),
